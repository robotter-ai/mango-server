    ...
    try {
      ...
      switch (decodedIx.name) {
        ...
        case "perpForceClosePosition":
          event = handlePerpForceClosePosition(
            decodedIx,
            ix,
            signature,
            blockTime,
        ...
        case "perpCancelAllOrders":
          event = handlePerpCancelAllOrders(
            decodedIx,
            ix,
            signature,
            blockTime,
...
function handleDeposit(
  ...
  const accounts = accountKeyIndexes.map(
    (index: number) => staticAccountKeys[index],
  );
  const [
    group,
    mangoAccount,
...
function handleWithdraw(
  ...
  const [group, mangoAccount, owner, bank, vault, oracle, tokenAccount] =
    accounts;
  const { amount } = decodedIx.data;
  const timestamp = blockTime ? blockTime * 1000 : Date.now();
...
function handlePerpSettlePnl(
  ...
  const [group, mangoAccount, perpMarket, accountA, accountB, oracle] =
    accounts;
  const timestamp = blockTime ? blockTime * 1000 : Date.now();
  const token = getPerpMarketSymbol(perpMarket.toBase58());
...
function handlePerpFill(
  ...
  const { makerOrderId, takerOrderId, makerFee, takerFee, price, quantity } =
    decodedIx.data;
  const timestamp = blockTime ? blockTime * 1000 : Date.now();
  const token = getPerpMarketSymbol(perpMarket.toBase58());
...
function handleTokenSwap(
  ...
  const [group, mangoAccount, owner, buyBank, sellBank, buyOracle, sellOracle] =
    accounts;

  const {
    maxBuyTokenToRelease,
    maxSellTokenToRelease,
    buyTokenIndex,
    sellTokenIndex,
  } = decodedIx.data;
  const timestamp = blockTime ? blockTime * 1000 : Date.now();
