import {
  ...
  PerpFillEvent,
  isLiquidationEvent,
  isPerpCancelAllOrdersEvent,
  isPerpCancelOrderEvent,
  isPerpFillEvent,
  isPerpForceClosePositionEvent,
  isPerpPlaceOrderEvent,
  isPerpSettleFeesEvent,
  isPerpSettlePnlEvent,
  isSwapEvent,
  isTradeEvent,
  isWithdrawEvent,
  isDepositEvent,
...
export function getEventsByMangoAccount(mangoAccount: string): MangoEvent[] {
  ...
  const perpCancelAllOrders =
    getPerpCancelAllOrdersEventsByMangoAccount(mangoAccount);
...
function getDepositWithdrawEventsByMangoAccount(
  mangoAccount: string,
...
function getLiquidationEventsByMangoAccount(
  mangoAccount: string,
): LiquidationEvent[] {
  const stmt = db.prepare(`
...
function getPerpCancelOrderEventsByMangoAccount(
  mangoAccount: string,
): PerpCancelOrderEvent[] {
  const stmt = db.prepare(`
...
function getPerpCancelAllOrdersEventsByMangoAccount(
  mangoAccount: string,
): PerpCancelAllOrdersEvent[] {
  const stmt = db.prepare(`
...
function getPerpFillEventsByMangoAccount(
  mangoAccount: string,
): PerpFillEvent[] {
  const stmt = db.prepare(`
...
function savePerpForceClosePositionEvent(
  event: PerpForceClosePositionEvent,
): number {
  const stmt = db.prepare(`
